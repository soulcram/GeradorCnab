...
@Setter
@Controller
public class CnabReativacaoController implements Serializable {
	...
	private List<TituloDto> titulosDesativados;
	...
	public void init() {
		...
		titulosDesativados = new ArrayList<>();
		cnab = new CnabDto();
	...
	public void selecionandoFundo() {
		...
		if(optional.isPresent()) {
			try {
				...
				movimentos = movimentoService.findAllMovimentosReativacao(base, layoutSelecionado);
				if(movimentos == null || movimentos.isEmpty()) {
					FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "", "Nenhum movimento encontrado para o Layout selecionado."));
					return;
				}
				atualizarTitulosEmEstoque();
	...
	private void atualizarTitulosEmEstoque() {
		...
		try {
			Base base = baseService.findById(baseSelecionada);
			titulosDesativados = movimentoService.findAllTituloDesativados( base,fundoSelecionado);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void addTitulo(TituloDto titulo) {
		...
		titulosDesativados.remove(titulo);
	...
	public void gerar() {
		try {
			...
			geradorCnab.gerar(cnab, "REATIVACAO", importacaoAutomatica, path);
