...
public class CnabRecompraParcialController implements Serializable {
	...
	public void selecionandoFundo() {
		...
		if(optional.isPresent()) {
			try {
				...
				movimentosBaixaRecompra = movimentoService.findAllMovimentosRecompraParcial(base, layoutSelecionado);
				movimentosAquisicaoRecompra = movimentoService.findAllMovimentosRecompraAquisicao(base, layoutSelecionado);
	...
	public void addTituloAquisicao() {
		...
		TituloDto titulo = new TituloDto(optionalCedente.isPresent() ? optionalCedente.get() : null, 
				...
				ValorAleatorioUtil.getTaxaDecimal(),
				taxaJuros,
				null,
				null,
				null,
				null,
				null,
				StringUtils.defaultIfNull(qtdeLastro,""),
				null
				);
