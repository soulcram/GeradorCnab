...
//				atualizarTitulosEmEstoque();
	...
	public void selecionandoLayout() {
		try {
			...
			cobrancas = fundoService.findCodCobrancas( base,fundoSelecionado);
			
			if(cobrancas.isEmpty()) {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "", "Não Existe codigo de cobrança cadastrados."));
				return;
			}
			...
			cobrancas.forEach(c -> {
				try {
					...
					if(!layoutsCobranca.contains(layout)) {
						...
						if(LayoutEnum.CNAB_400_COBRANCA_ID.equals(layout)) {
							layoutsCobranca.add(LayoutEnum.CNAB_240_COBRANCA_ID);
						}
					}
					
					BancoDto banco = bancoService.findOneByNumBanco(base, c.getNuBanco());
			...
			movimentos = movimentoService.findAllMovimentos(base, (layoutSelecionado == null ? layoutsCobranca.get(0).getCdLayout():layoutSelecionado) , true);

			if(movimentos.isEmpty()) {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "", "Não Existe layout movimentos cadastrados."));
	...
	public void atualizarTitulosEmEstoque() {
		...
		try {
			...
			if(optionalMovimento.isPresent()) {
				...
				titulosEmEstoque = movimentoService.findAllTituloEmEstoqueCobranca( base,fundoSelecionado,movimentoDto.getIdMovimento(), LayoutEnum.parse(layoutSelecionado));
