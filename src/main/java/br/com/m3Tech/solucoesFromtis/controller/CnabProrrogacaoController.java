...
public class CnabProrrogacaoController implements Serializable {
	...
	public void selecionandoFundo() {
		...
		if(optional.isPresent()) {
			try {
				...
				movimentos = movimentoService.findAllMovimentosProrrogacao(base, layoutSelecionado);
				atualizarTitulosEmEstoque();
	...
	private void atualizarTitulosEmEstoque() {
		...
		try {
			Base base = baseService.findById(baseSelecionada);
			titulosEmEstoque = movimentoService.findAllTituloEmEstoqueByFundo( base,fundoSelecionado,true);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private boolean validarDataVencimento() {
		...
		try {
			...
			diasMaxProrrogacao = fundoService.findDiasMaxProrrogacao(base,fundoSelecionado);
		}catch(Exception e) {
