...
@Service
public class SimularLiquidacaoParcialPortalImpl implements ISimularImportacaoPortal, Serializable {
	
	private static final Logger logger = LoggerFactory.getLogger(SimularLiquidacaoParcialPortalImpl.class);
	...
	public void gerar(ImportacaoSimuladaDto importacaoSimuladaDto) throws Exception {
		...
		if(TipoMovimentacao.LIQUIDACAO_PARCIAL.equals(importacaoSimuladaDto.getTipoMovimentacao())) {
			...
			for(int i = 0; i < importacaoSimuladaDto.getQuantArquivos(); i++) {
				...
				if(findAllTituloEmEstoque == null || findAllTituloEmEstoque.isEmpty()) {
					...
					throw new BussinesException("Nenhum titulo disponível");
				}
				...
				List<MovimentoDto> movimentos = movimentoService.findAllMovimentosParcial(base, fundoAtual.getLayoutAquisicao());
				...
				File file = geradorCnab.gerar(cnab, "LIQUIDACAO_PARCIAL_PORTAL", false, path.toString());
