...
@Service
public class SimularBaixaPortalImpl implements ISimularImportacaoPortal, Serializable {
	
	private static final Logger logger = LoggerFactory.getLogger(SimularBaixaPortalImpl.class);
	...
	public void gerar(ImportacaoSimuladaDto importacaoSimuladaDto) throws Exception {
		...
		if(TipoMovimentacao.BAIXA.equals(importacaoSimuladaDto.getTipoMovimentacao())) {
			...
			for(int i = 0; i < importacaoSimuladaDto.getQuantArquivos(); i++) {
				...
				List<MovimentoDto> movimentos = movimentoService.findAllMovimentosBaixa(base, fundoAtual.getLayoutAquisicao());
				...
				File file = geradorCnab.gerar(cnab, "BAIXA_PORTAL", false, path.toString());
