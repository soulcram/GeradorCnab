...
@Service
public class SimularRecompraPortalImpl implements ISimularImportacaoPortal, Serializable {
	
	private static final Logger logger = LoggerFactory.getLogger(SimularRecompraPortalImpl.class);
	...
	public void gerar(ImportacaoSimuladaDto importacaoSimuladaDto) throws Exception {
		...
		if(TipoMovimentacao.RECOMPRA.equals(importacaoSimuladaDto.getTipoMovimentacao())) {
			...
			for(int i = 0; i < importacaoSimuladaDto.getQuantArquivos(); i++) {
				...
				List<MovimentoDto> movimentosRecompraBaixa = movimentoService.findAllMovimentosRecompraBaixa(base, fundoAtual.getLayoutAquisicao());
				if(movimentosRecompraBaixa.isEmpty()) {
				...
				titulos.forEach(t -> t.setMovimento(movimentosRecompraBaixa.get(0)));
				...
				File file = geradorCnab.gerar(cnab, "RECOMPRA_PORTAL", false, path.toString());
