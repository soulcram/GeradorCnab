...
public class GerarCnabAquisicao extends JPanel {
	...
	public GerarCnabAquisicao() {
		try {
			...
			this.add(new Label("Gerar Cnab Aquisição", 10, 10, 500, 20, 14, Color.BLACK));
			...
			dataVencimento = new Text(580, 150, 100, 20);
...
//				"Seu Numero","Valor Título","Cedente","Sacado"
	...
	private void preencherComboMovimento() throws Exception {
		List<MovimentoDto> movimentos = movimentoService.findAllMovimentos(Conexao.getConnection((Base)cbBase.getSelectedItem()), ((LayoutEnum)cbLayout.getSelectedItem()).getCdLayout());
